...
router.put("/:act_id/user/:user_id/approve", async function(req, res, next) {
  try {
    ...
    const promised_act_change = Act.findByIdAndUpdate(req.params.act_id, {
      $push: {
    ...
    await mail.sendMail(
      ...
      `Click <a href='${process.env.website}acts/${
        req.params.act_id
      }'>here</a> to view the approved act`
    );
...
router.put("/:act_id/user/:user_id/disapprove", async function(req, res, next) {
  try {
    ...
    await mail.sendMail(
      ...
      `<p>Reason: ${req.body.comments}</p><p>Click <a href='${
        process.env.website
      }acts/${req.params.act_id}'>here</a> to change your proof</p>`
    );
    ...
    const promised_user_change = User.findOneAndUpdate(
      ...
      async function(err, numberAffected, rawResponse) {
        if (!numberAffected) {
          ...
          await User.findByIdAndUpdate(user.id, { $push: { acts: act } });
        }
      }
    );
...
router.put("/:reward_id/user/:user_id/request_reward", async function(
  ...
  try {
    ...
    const promised_user_change = User.findByIdAndUpdate(req.params.user_id, {
      $push: { rewards: reward },
